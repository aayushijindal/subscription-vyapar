    ...
    return (
      <div className="min-h-screen bg-background p-4 sm:p-6 lg:p-8 font-sans">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-surface rounded-2xl shadow-sm border border-border/60 p-5 mb-6">
          <div>
            <h1 className="text-[18px] font-black text-text-primary tracking-tight uppercase">PURCHASE ENTRY LIST</h1>
            <p className="text-[11px] font-bold text-text-muted uppercase mt-0.5">Manage Purchase Invoices</p>
          </div>
              ...
              <table className="w-full text-left text-[11px] font-bold">
                ...
                <tbody className="divide-y divide-border">
                  {purchases.map((purchase) => {
                    ...
                    return (
                    <tr key={purchase.id} className="hover:bg-input/50 transition-colors">
                      ...
                      <td className="py-4 px-4 text-text-primary">{purchase.invoice_no || '-'}</td>
                      <td className="py-4 px-4 text-text-primary uppercase">{partyName || '-'}</td>
                      <td className="py-4 px-4 text-text-secondary text-right">₹ {purchase.grand_total ? Number(purchase.grand_total).toLocaleString('en-IN') : '0'}</td>
                      <td className="py-4 px-4 text-center">
                        <button className="bg-background text-text-primary border border-border px-3 py-1.5 rounded text-[10px] uppercase font-black hover:bg-slate-200 transition-colors shadow-sm">GRN</button>
                      </td>
                      <td className="py-4 px-4 text-center">
                        <div className="flex justify-center items-center gap-2">
                          ...
                          <button onClick={() => handleEdit(purchase.id)} className="text-primary hover:text-primary bg-primary-light p-1.5 rounded transition-colors"><Pencil className="w-3.5 h-3.5" /></button>
  ...
  return (
    <div className="min-h-screen bg-background p-4 sm:p-6 lg:p-8 font-sans">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-[20px] font-black text-text-primary tracking-tight uppercase">{editingId ? 'Edit Purchase Entry' : 'Create Purchase Entry'}</h1>
      </div>
        ...
        <div className="p-6 sm:p-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-x-6 gap-y-5 mb-10">
            ...
            <div className="space-y-1.5">
              <label className="text-[11px] font-bold text-text-secondary uppercase tracking-wider">Purchase No. (Auto)</label>
            ...
            <div className="space-y-1.5">
              <label className="text-[11px] font-bold text-text-secondary uppercase tracking-wider">RCM Purchase</label>
              <select value={rcmPurchase} onChange={(e) => setRcmPurchase(e.target.value)} className="w-full bg-surface border border-border focus:border-primary focus:ring-1 focus:ring-[#2b5f9d] rounded-lg p-2.5 text-[13px] text-text-secondary transition-all outline-none">
                <option>NO</option>
                <option>YES</option>
              </select>
            </div>
          ...
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-[13px] font-black text-text-primary tracking-wide uppercase">Purchase Items Listing</h2>
          ...
          <div className="flex justify-end">
            <div className="w-[400px] border border-border/60 rounded-xl overflow-hidden">
              <div className="bg-background py-3 px-5 border-b border-border/60">
                <h3 className="text-[12px] font-black text-text-primary tracking-wide uppercase">Purchase Bill Summary</h3>
              </div>
              ...
              <div className="p-5 space-y-4">
                ...
                <div className="flex justify-between items-center text-[12px] font-bold text-text-secondary uppercase">
                  ...
                  <input 
                    ...
                    placeholder="0" 
                    className={`w-24 text-right border border-border rounded p-1.5 text-[12px] outline-none ${rcmPurchase === 'YES' || loadingType === 'AUTO' ? 'bg-background cursor-not-allowed' : 'focus:border-primary'}`} 
                  />
                </div>
