    ...
    <div className="min-h-screen bg-[#F8F9FC] p-4 sm:p-6 lg:p-8 font-sans">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white rounded-xl shadow-sm border border-slate-200/60 p-5 mb-6">
        <div>
          ...
          <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Manufacturing Ledger Entries</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="relative">
            <input type="text" placeholder="Search item or remarks..." className="w-[250px] bg-slate-50 border border-slate-200 focus:border-[#4338CA] focus:bg-white rounded-lg pl-9 pr-4 py-2.5 text-[12px] outline-none transition-all" />
        ...
        <table className="w-full text-left text-[11px]">
          ...
          <tbody className="divide-y divide-slate-100">
            {loading ? (
              ...
              <tr><td colSpan={8} className="py-8 text-center text-slate-500">No finish goods entries found.</td></tr>
            ) : (
              data.map((item: any, i: number) => (
                <tr key={item.id || i} className="hover:bg-slate-50/50 transition-colors">
                  <td className="py-4 px-5">
                    ...
                    <div className="text-[10px] text-slate-400 mt-0.5">{item.id ? `FG/${item.id}` : '--'}</div>
                  </td>
                  <td className="py-4 px-5">
                    <div className="font-bold text-[#1E293B]">{item.party_name || '--'}</div>
                    <div className="text-[10px] text-slate-400 mt-0.5 uppercase">BILL: {item.bill_no || '--'}</div>
                  </td>
                  ...
                  <td className="py-4 px-5 text-slate-500">{item.remarks || '--'}</td>
                </tr>
              ))
            )}
          </tbody>
